...
patterns, and callers with the code-search MCP or the CLI provided by
codebase-memory-search. Load that skill for project selection, query routing,
and revision/coverage checks. Use rg
...
function getProjectReuseContext(cwd) {
...
function contextFor(payload) {
  ...
  const context = getProjectReuseContext(payload?.cwd);
  if (context) return { hookSpecificOutput: { hookEventName: event, additionalContext: context } };
}

module.exports = { getProjectReuseContext };
